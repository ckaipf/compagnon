...
def parse_kraken_result_file(path: str, kwargs) -> pandas.DataFrame:
    fieldnames = [field["fieldname"] for _, field in sorted(kwargs["kraken_result_file_fields"].items(), key=lambda x: x[0])]
    dypes = {v["fieldname"]: v["dtype"] for k, v in kwargs["kraken_result_file_fields"].items()}
